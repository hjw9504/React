...
export default function Friends() {
  ...
  const tabs: {key: TabType; label: string; count: number; icon: string}[] = [
    ...
    {key: "following", label: "팔로잉", count: onlyFollowing.length, icon: "➡️"},
  ];
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 transition-colors duration-200">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="bg-gradient-to-r from-orange-200 via-rose-200 to-pink-200 dark:from-orange-900/40 dark:via-rose-900/40 dark:to-pink-900/40 rounded-2xl p-5 shadow-sm border border-orange-100 dark:border-orange-800/30">
          <h1 className="text-lg font-bold mb-4 text-gray-700 dark:text-gray-200">나의 팔로우 현황</h1>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/60 dark:bg-white/10 rounded-xl p-3 text-center">
              <p className="text-3xl font-extrabold text-orange-500 dark:text-orange-300">{friends.length}</p>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-300 mt-0.5">💚 친구</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">서로 팔로우</p>
            </div>
            <div className="bg-white/60 dark:bg-white/10 rounded-xl p-3 text-center">
              <p className="text-3xl font-extrabold text-rose-400 dark:text-rose-300">{onlyFollowing.length}</p>
              <p className="text-sm font-medium text-gray-600 dark:text-gray-300 mt-0.5">➡️ 팔로잉</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">내가 팔로우 중</p>
            </div>
          </div>
        </div>
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200
                ${activeTab === tab.key
                  ? "bg-gradient-to-r from-orange-300 to-rose-300 dark:from-orange-500/70 dark:to-rose-500/70 text-white shadow-md"
          ...
          {displayed.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow p-12 flex flex-col items-center text-center">
              <span className="text-5xl mb-3">
                {activeTab === "friends" ? "💚" : activeTab === "following" ? "➡️" : "👥"}
              </span>
              <p className="text-gray-500 dark:text-gray-400 font-medium">
                {activeTab === "friends" ? "아직 서로 팔로우한 친구가 없어요" :
                 activeTab === "following" ? "팔로잉하는 사람이 없어요" :
                 "아직 팔로우한 사람이 없어요"}
              </p>
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-gray-800 rounded-2xl shadow p-4 transition-all duration-200
                    ${isFriend ? "border-l-4 border-green-400" : "border-l-4 border-blue-300"}`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <img
                      ...
                      className={`flex-shrink-0 w-12 h-12 rounded-full object-cover ring-2 ${isFriend ? "ring-green-300 dark:ring-green-600" : "ring-blue-200 dark:ring-blue-700"}`}
                    />

                    {/* 정보 */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-bold text-gray-800 dark:text-gray-100">@{f.follow_user_id}</p>
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                        {formatDate(f.register_time)} 팔로우
                      </p>
                    </div>
                    ...
                    {!isConfirming ? (
                      <button
                        ...
                        className="flex-shrink-0 text-xs px-3 py-1.5 rounded-xl border border-gray-200 dark:border-gray-600
                                   ...
                                   dark:hover:border-red-600 dark:hover:text-red-400 transition-all duration-200"
                      >
                        언팔로우
                      </button>
                    ) : (
